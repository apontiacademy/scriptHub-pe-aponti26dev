...
### 0.20.0.dev12 - 2026-07-20 - (PR#66)

- **Added**: `ERRORS.md`, documentando o esquema de 6 códigos de saída (0-5) e a tabela de classificação de cada `raise` nos pacotes de script; handler global único de erros (`_executar_com_tratamento_global` em `cli.py`), que captura qualquer `ErroScriptHub`/exceção genérica, decide o código de saída e loga a mensagem final — nenhum outro ponto do código (comandos, `executar_script`, `_carregar_config`) chama `sys.exit`/`typer.Exit`/loga o erro final diretamente; nível de log `sucesso` (`log.sucesso`, grava `SUCCESS` em `logs/scripthub.log`), usado uma única vez ao fim de cada pipeline
- **Changed**: `ErroConfiguracao`/`ErroUsoCLI`/`FalhaParcial`/`ErroIntegracao` (códigos 2-5) substituem exceções genéricas em vários pontos de `auditar_frequencias`, `auditar_relatorios`, `auditar_softskills`, `compilacao_de_relatorios` e `torpedo_de_forum` — Moodle, Google Sheets/Drive e `pentefino` retornando algo inesperado agora viram `ErroIntegracao` (5) em vez de erro genérico (1); flag `--debug` mostra traceback completo só para erros não classificados (código 1), nunca para erros já classificados (2-5); mensagens de conclusão específicas de cada script (`"Escopo N finalizado com sucesso!"`, `"PIPELINE EXECUTADO E CONCLUÍDO COM SUCESSO ABSOLUTO!"`, `"AUTOMAÇÃO DE FÓRUNS CONCLUÍDA COM SUCESSO!"`) removidas em favor de uma mensagem final única e padronizada; `typer` travado em `<0.27` (patches internos de `_i18n.py` frágeis a atualizações da lib)
- **Fixed**: painel de erro duplicado ao validar `--passo` inválido (validação movida para fora do `try` em `executar_script`); `scripthub config --limpar -s script-inexistente` deixa de retornar silenciosamente com código 0 e passa a levantar `ErroUsoCLI` (código 3); falha individual de geração de PDF em `compilacao_de_relatorios` volta a ser logada como erro (`log.erro`), não aviso; `auditar_relatorios`: `SystemExit(0)` do `pentefino` (sucesso interno do Core) deixa de ser convertido incorretamente em `ErroIntegracao`
