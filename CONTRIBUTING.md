...
- **Título**:
  - Squash normal (feature/fix/etc.): trocar o prefixo de tipo (`tipo(escopo):`) pela versão — `[vX.Y.Z(.devN)] título (#pr)`, com `x.y.z.devN` em `dev` e `x.y.z` em `nightly`/`main`.
  - Squash de um commit de changelog/release (`chore(changelog)`/`chore(release)`, ver "Commits de changelog/release" abaixo): a PR contra o branch que é o alvo real da promoção usa só `[vX.Y.Z] (#pr)`, sem texto descritivo — a entrada correspondente em `CHANGELOG.md`/`SNAPSHOTS.md` já documenta o que mudou. As PRs-eco contra os demais branches de destino (que só replicam essa atualização de changelog, sem serem o alvo real daquela promoção) usam `Promoção de vX.Y.Z para <branch-alvo> (#pr)` — sem colchetes, já que a versão não está sendo lançada *naquele* branch. Ver os exemplos concretos nos bullets "Promoção `dev → nightly`"/"Promoção `nightly → main`" em "Commits de changelog/release" abaixo.
  - Em ambos os casos, o `(#pr)` só vem preenchido sozinho na sugestão inicial da caixa; ao editar o título, é preciso digitar `(#pr)` de volta manualmente.
...
- **Promoção `dev → nightly`** — branch dedicada a partir de `dev`, com o commit `chore(changelog)` que: (1) remove de `SNAPSHOTS.md` as entradas da versão sendo promovida; (2) adiciona esse mesmo conteúdo ao `CHANGELOG.md`, como `### [x.y.z] - data` aninhado dentro do `## [Unreleased]` (Keep a Changelog permite agrupar por versão mesmo dentro de "não lançado" — é assim que uma versão em `nightly` aparece documentada antes de virar release oficial em `main`); e (3) troca a versão em `pyproject.toml`/`uv.lock` do formato de snapshot para a versão plana (`x.y.z.devN` → `x.y.z`). Essa branch abre **duas PRs com o mesmo diff**: uma contra `nightly` e uma contra `dev` — as duas ficam sincronizadas quanto ao que já foi promovido (sem a PR contra `dev`, `CHANGELOG.md`/`SNAPSHOTS.md` de `dev` ficam defasados indefinidamente). `nightly` é o alvo real dessa promoção: a PR contra `nightly` usa o título `[vX.Y.Z] (#pr)`; a PR-eco contra `dev` usa `Promoção de vX.Y.Z para nightly (#pr)`.
- **Promoção `nightly → main`** — squash manual gera um commit novo em cada branch de destino mesmo quando o diff é idêntico (ver "Squash manual em dev/nightly/main" acima), então a promoção `dev → nightly` anterior deixa `nightly` e `dev` com conteúdo sincronizado mas **sem ancestralidade linear entre si** (`git merge-base nightly dev` volta para antes daquela promoção, não para o ponto de sincronização). Como `dev` segue avançando sozinho entre uma promoção e outra — inclusive com mudanças estruturais (renomeações de pacote/arquivo) —, um branch único a partir de `nightly` tende a gerar conflitos de merge espúrios contra `dev` (add/add, modify/delete, arquivo "recriado" num caminho que `dev` já renomeou) sem relação nenhuma com o conteúdo do release. Por isso essa promoção usa **dois branches dedicados**, não um só:
  - Um a partir de `nightly`, que promove a entrada `### [x.y.z] - data` de dentro do `## [Unreleased]` para uma seção própria de topo, `## [x.y.z] - data` (os `####` de Added/Changed/Fixed/Removed dentro dela sobem para `###`), mantendo um `## [Unreleased]` vazio acima para a próxima leva; corrige `pyproject.toml`/`uv.lock` para `x.y.z` quando aplicável (sempre em `main`; conferido/corrigido em `nightly` se necessário). Abre **duas PRs com o mesmo diff**: contra `main` (release oficial) e contra `nightly`. `main` é o alvo real: a PR contra `main` usa `[vX.Y.Z] (#pr)`; a PR-eco contra `nightly` usa `Promoção de vX.Y.Z para main (#pr)`.
  - Outro a partir de `dev`, aplicando isoladamente a mesma edição de `CHANGELOG.md` (o conteúdo da entrada já é idêntico ao de `nightly` nesse ponto — só a posição no arquivo muda) e nada mais: não mexe em `pyproject.toml`/`uv.lock`, já que `dev` normalmente está adiantado no `devN` da próxima versão. Abre **uma PR** contra `dev`, também uma PR-eco: mesmo título `Promoção de vX.Y.Z para main (#pr)`.
