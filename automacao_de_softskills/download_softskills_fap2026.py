...
def login(session):
    ...
    if not token_input:
        print("Erro: Token de login não encontrado.")
        return
...
def get_turmas(session):
    ...
    for a in soup.find_all("a", href=True):
        text = a.text.strip()
        href = a.get("href")
        if href and "BootCamp Turma" in text and "course/view.php" in href:
...
def download_csv(session, quiz_id):
    ...
    if not sk:
        return None
    dl = session.post(
        f"{MOODLE_URL}/mod/quiz/report.php",
        data={
            "sesskey": sk["value"],
            "download": "csv",
            "id": quiz_id,
...
def main():
    ...
    ss_fieldnames = (
        ["Turma", "Nome Completo", "E-mail"] + [f"Nota {label}" for label, _ in ACTIVITIES] + ["Nota Geral Soft Skills"]
    )
    ...
    sem_notas = [
        t
        for t in sorted(turmas.keys())
        if not any(r.get(f"Nota {label}", "") for r in rows_out if r["Turma"] == t for label, _ in ACTIVITIES)
    ]
    if sem_notas:
